...
class Graph:
  ...
  def explore(self):
    ...
    path_total = 0
    print("\nStarting off at the %s" % (current_room))
    ...
    while current_room != 'treasure room':
      ...
      for connected_room, weight in node.edges.items():
        ...
        print("Enter %s for %s: %s cost" % (key, connected_room, weight))
      
      valid_choices = [room[:1] for room in node.edges.keys()]
      print("\nYou have accumulated: %s cost" % (path_total))
      choice = input("\nWhich room do you move to? ")
      
      if choice not in valid_choices:
        print("Please, select from these letters %s" % (valid_choices))
    ...
    print("Made it to the treasure room with %s cost" % (path_total))
